...
OptionChoice::OptionChoice(size_t cap)
    : capacity(cap), size(0), stages(cap > 0 ? new size_t[cap] : nullptr) {
}

OptionChoice::OptionChoice(const OptionChoice& other)
...
OptionChoice& OptionChoice::operator=(const OptionChoice& other) {
...
OptionChoice::OptionChoice(OptionChoice&& other) noexcept
...
OptionChoice& OptionChoice::operator=(OptionChoice&& other) noexcept {
    if (this == &other) return *this;
    delete[] stages;
    stages = other.stages;
    capacity = other.capacity;
...
bool OptionChoice::addChoice(size_t index) {
    if (size >= capacity) {
...
size_t OptionChoice::getSize() const {
    return size;
}

size_t OptionChoice::operator[](size_t i) const {
    return stages[i];
}

void OptionChoice::serialize(std::ostream& out) const {
...
OptionChoice OptionChoice::deserialize(std::istream& in) {
    size_t size = 0;
